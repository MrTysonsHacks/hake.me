...
Bristleback.optionVersion = Menu.AddOption({ "Hero Specific","Bristleback. Script By Neil Tyson" }, "Version", "Author: Twitch", 1, 1, 1)
...
function Bristleback.Combo()
	...
	--Nasal Goo
	if Ability.IsCastable(goo, myMana) and Menu.IsEnabled(Bristleback.optionGoo) and not NPC.HasState(hero, Enum.ModifierState.MODIFIER_STATE_MAGIC_IMMUNE) and hero and NPC.IsEntityInRange(hero, myHero, Ability.GetCastRange(goo)) and Menu.IsKeyDown(Bristleback.optionKey) then Ability.CastTarget(goo, hero) return end

	--Quill
	if Ability.IsCastable(quill, myMana) and Menu.IsEnabled(Bristleback.optionQuill) and NPC.IsEntityInRange(hero, myHero, Ability.GetCastRange(quill)) and Menu.IsKeyDown(Bristleback.optionKey) then Ability.CastPosition(quill, NPC.GetAbsOrigin(hero)) return end
